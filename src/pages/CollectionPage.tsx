...
export default function CollectionPage() {
  ...
  const collectionRestaurants = restaurants.filter(collection.filter);
  ...
  return (
    <>
      <SEOHead
        ...
        description={`${collection.description} ${collectionRestaurants.length} curated spots on Ono Oahu.`}
        image={`https://www.onooahu.com${collection.image}`}
        schema={schema}
      />
      <PageLayout>
        <div className="relative h-[400px] md:h-[500px] overflow-hidden">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-8 md:p-16 max-w-[1280px] mx-auto">
            ...
            <h1 className="text-4xl md:text-6xl text-white font-serif leading-[1.1] tracking-tight mb-4">{collection.title}</h1>
            <p className="text-base md:text-lg text-[#f5ede4] font-sans max-w-2xl leading-relaxed">{collection.description}</p>
          </div>
        </div>

        <MonetizeBanner context="collection" />

        <section className="py-16 px-6 md:px-12">
          <div className="max-w-[1280px] mx-auto">
            <div className="flex items-center justify-between mb-8">
              <p className="text-sm text-[#666666] font-sans">{collectionRestaurants.length} spots curated by our editors</p>
              <Link to="/discover" className="text-sm font-serif italic text-[#1a1a1a] underline underline-offset-4 hover:text-[#d4a574] transition-colors">
                View All Restaurants
              </Link>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {collectionRestaurants.map((r) => (
                <Link
                  key={r.id}
                  to={`/restaurants/${r.id}`}
                ...
                >
                  ...
                  <div className="p-5">
                    ...
                    <h3 className="text-xl text-[#1a1a1a] font-serif mb-2 leading-tight group-hover:text-[#d4a574] transition-colors">{r.name}</h3>
                    <p className="text-sm text-[#666666] font-sans leading-relaxed mb-4 line-clamp-3">{r.description}</p>
                    ...
                    {getReservationLink(r.id) && (
                      <a
                        ...
                        className="inline-flex items-center gap-1.5 text-xs font-sans text-[#d4a574] hover:text-[#1a1a1a] transition-colors"
                    ...
                    <div className="border-t border-[#f0ebe4] pt-3">
                      <p className="text-[11px] uppercase tracking-[0.06em] text-[#d4a574] font-sans mb-1.5">Must Try</p>
                      <div className="flex flex-wrap gap-2">
                        {r.mustTry.map((item, idx) => (
                          <span key={item} className="text-xs text-[#666666] font-sans">
                            {item}{idx < r.mustTry.length - 1 && <span className="text-[#d4a574] ml-2">+</span>}
                          </span>
                        ))}
                      </div>
                    </div>
            ...
            {/* AdSense - Bottom of collection */}
            <AdSenseSlot slot="collection-bottom" />
          </div>
        </section>
      </PageLayout>
    </>
